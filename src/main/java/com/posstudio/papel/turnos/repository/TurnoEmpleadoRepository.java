...
public interface TurnoEmpleadoRepository extends JpaRepository<TurnoEmpleado, Long> {
        ...
        // NUEVO: Verifica si un empleado tiene un turno activo sin salida
        @Query("""
                        SELECT CASE WHEN COUNT(te) > 0 THEN true ELSE false END
                        FROM TurnoEmpleado te
                        WHERE te.empleado.id = :empleadoId
                        AND te.horaSalida IS NULL
                        AND te.turno.estadoTurno = 'ABIERTO'
                        """)
        boolean existsEmpleadoActivoEnTurnoAbierto(@Param("empleadoId") Long empleadoId);
